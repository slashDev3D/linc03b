...
// 윗글을 얻음
$sql = " select wr_id, wr_subject, wr_datetime from g5_write_event_info where wr_is_comment = 0 and wr_num < '{$write['wr_num']}' and wr_1 = '1' and mb_id not in('test','test2','monq') order by wr_num desc, wr_reply desc limit 1 ";
$prev = sql_fetch($sql);

// 아래글을 얻음
$sql = " select wr_id, wr_subject, wr_datetime from g5_write_event_info where wr_is_comment = 0 and wr_num > '{$write['wr_num']}' and wr_1 = '1' and mb_id not in('test','test2','monq') order by wr_num, wr_reply limit 1 ";
$next = sql_fetch($sql);
